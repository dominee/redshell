...
def resolve(args):
    ...
    if len(args) > 0:
        ...
        if len(args) == 2:
            ...
            try:
                ...
                sys.stdout.write(PLUS+"Nameserver: "+args[1]+"\n")
    ...
    # Check for A
    sys.stdout.write(PLUS+"Resolving "+colored(h,'white')+" \n")
    for ip in my_resolver.query(h, 'A'):
        try:
            hostname = my_resolver.query(dns.reversename.from_address(str(ip)), 'PTR')[0]
            sys.stdout.write(STAR+"IP: "+colored(str(ip),'white')+" -> "+str(hostname)+"\n")
        except:
            sys.stdout.write(STAR+"IP: "+colored(str(ip),'white')+"\n")

    # Check for CNAME
    try:
        cname = my_resolver.query(h, 'CNAME')[0]
        sys.stdout.write(STAR+""+h+" is an alias for "+colored(str(cname),'white')+"\n")
    except:
        pass
    ...
    try:
        for mx in my_resolver.query(h, 'MX'):
            try:
                ...
                hostname = my_resolver.query(dns.reversename.from_address(str(ip)),'PTR')[0]
                sys.stdout.write(LS+"MX: "+str(mx)+" -> "+str(ip)+" -> "+str(hostname)+"\n")
            except:
                sys.stdout.write(LS+"MX: "+str(mx)+" -> "+str(ip)+"\n")
    except:
        pass
    ...
    try:
        for ns in my_resolver.query(h, 'NS'):
            ...
            try:
                ...
                hostname = my_resolver.query(dns.reversename.from_address(str(ip)), 'PTR')[0]
                sys.stdout.write(LS+"NS: "+str(ns)+" -> "+str(ip)+" -> "+str(hostname))
            except:
                sys.stdout.write(LS+"NS: "+str(ns)+" -> "+str(ip))
    ...
    try:    
        for aaaa in my_resolver.query(h, 'AAAA'):
            sys.stdout.write(LS+"AAAA: "+str(aaaa)+"\n")        
    except:
        pass
    ...
    try:    
        for txt in my_resolver.query(h, 'TXT'):
            sys.stdout.write(LS+"TXT: "+str(txt)+"\n")        
    except:
        pass
